...
Assets.prototype.compile = function (callback) {
  ...
  if (this.manifest) {
    this.manifest.compile(this.options.precompile, callback);
  }
  else {
...
Assets.prototype.helper = function (tagWriter, ext) {
  ...
  return function (path) {
    ...
    var getTag = function (asset) {
      ...
      var isString = typeof asset === 'string';
      var path;

      if (isString) {
        path = servePath + "/" + asset;
      } else {
        path = servePath + "/" + asset.logicalPath;
      }
      ...
      var finalPath = isString ? path : path.replace(/(\.[^.]+)$/, "-" + asset.digest + "$1");

      return tagWriter(finalPath);
    };
